    ...
    serialized_data = [
        {
            ...
            "view_results": poll.author == request.user
            or not poll.is_active
            or Vote.objects.filter(poll=poll, user=request.user).exists(),
        }
        for poll in data
    ]
    ...
    if request.method == "POST":
        try:
            ...
            if not question or not choices:
                return JsonResponse(
                    {"error": "Invalid data: Missing question or choices"}, status=400
                )
            ...
            return JsonResponse(
                {"message": "Poll created successfully!", "status": 200}, status=200
            )
...
def update(request, poll_id):
    if request.method == "POST":
        try:
            if request.user != Poll.objects.get(pk=poll_id).author and not (
...
def vote(request, poll_id):
    ...
    if vote:
        return JsonResponse(
            {"status": "error", "message": "You have already voted."}, status=400
        )
    if poll.author == request.user:
        return JsonResponse(
            {"status": "error", "message": "Poll author cannot vote."}, status=400
        )
    if not poll.is_active:
        return JsonResponse(
            {"status": "error", "message": "Poll is not active."}, status=400
        )
    ...
    if request.method == "POST":
        try:
            ...
            if not choice_id:
                return JsonResponse(
                    {"status": "error", "message": "Choice is required."}, status=400
                )
            ...
            return JsonResponse(
                {"status": "success", "message": "Vote recorded successfully."}
            )
        except json.JSONDecodeError:
            return JsonResponse(
        ...
        return JsonResponse(
            {"poll": poll.question, "choices": choices, "status": "success"}
        )
...
def ajax_poll_results(request, poll_id):
    ...
    if not vote and poll.author != request.user and poll.is_active:
        return JsonResponse(
            {"error": "You need to vote before viewing the results."}, status=403
        )

    choices_data = [
        {"choice_text": choice.choice_text, "vote_count": choice.vote_count}
